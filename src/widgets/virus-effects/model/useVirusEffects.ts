...
import { VIRUS_EFFECTS_CONFIG } from "@shared/config/virus";
...
const createToggleInterval = (
  ...
  const baseInterval =
    config.interval + (config.randomOffset ? Math.random() * config.randomOffset : 0);
  return setInterval(() => {
    setter(true);
...
export const useVirusEffects = (isActive: boolean, virusType: VirusType): UseVirusEffectsResult => {
  ...
  useEffect(() => {
    ...
    if (config.adBanners) {
      const bannerInterval =
        ...
        (config.adBanners.randomOffset ? Math.random() * config.adBanners.randomOffset : 0);
