import { AD_BANNERS_SET_1, AD_BANNERS_SET_2, SIDE_BANNERS } from "@shared/config/virus";
  ...
  return (
    <>
      {["tl", "tr", "bl", "br"].map(position => (
        <div key={position} className={`adware-banner-corner adware-banner-corner-${position}`}>
      ...
      {adBanner && <AdBanner bannerIndex={adBannerType} config={AD_BANNERS_SET_1[adBannerType]} />}

      {adBanner2 && (
        <AdBanner bannerIndex={adBanner2Type + 6} config={AD_BANNERS_SET_2[adBanner2Type]} />
      )}
    </>
  );
};
