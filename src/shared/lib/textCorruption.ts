...
const corruptChar = (char: string, corruptionLevel: number): string => {
  ...
  if (random < corruptionLevel * 0.3) {
    return RANDOM_CORRUPTION_CHARS[Math.floor(Math.random() * RANDOM_CORRUPTION_CHARS.length)];
  }
...
export const corruptText = (text: string, corruptionLevel: number = 0.3): string => {
  if (corruptionLevel <= 0) return text;
  if (corruptionLevel >= 1) {
    return text
      .split("")
      .map(
        () => RANDOM_CORRUPTION_CHARS[Math.floor(Math.random() * RANDOM_CORRUPTION_CHARS.length)]
      )
      .join("");
  }
