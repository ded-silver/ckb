...
import { updateCommandHistoryEntry } from "../commandHistory";
    ...
    (result: CommandResult, isHackCommand: boolean) => {
      ...
      const addNextLine = () => {
        if (currentIndex < result.output.length && currentCommandRef.current) {
          ...
          if (isHackCommand && totalLines > 0) {
            const progress = Math.min(Math.floor(((currentIndex + 1) / totalLines) * 100), 100);
          ...
          typingTimeoutRef.current = setTimeout(addNextLine, delay) as unknown as number;
        } else {
    ...
    (result: CommandResult, onComplete?: () => void) => {
      ...
      const addDestroyLine = () => {
        if (destroyIndex < result.output.length && currentCommandRef.current) {
          ...
          typingTimeoutRef.current = setTimeout(addDestroyLine, delay) as unknown as number;
        } else {
          setIsTypingOutput(false);
          typingTimeoutRef.current = null;
          if (onComplete) {
      ...
      typingTimeoutRef.current = setTimeout(addDestroyLine, 50) as unknown as number;
    },
    [setCommandHistory, setIsTypingOutput, currentCommandRef, typingTimeoutRef]
  );
