...
export const utilityCommands: Record<string, CommandFunction> = {
  ...
  weather: async (args) => {
    try {
      const { getWeather, getMyIP } = await import("../utils/api");
      ...
      return [
        ...
        "",
        `Error: ${error?.message || "Unable to fetch real weather data"}`,
  ...
  dice: (args) => {
    ...
    if (isNaN(sides) || sides < 2) {
      return [
        "Usage: dice [sides]",
        "Example: dice 6 (default) or dice 20",
        "",
      ];
    }
  ...
  joke: () => {
    const jokes = [
      ...
      {
        setup:
          "Why did the Python programmer not respond to the function call?",
