...
export const crackCommand: CommandFunction = (args) => {
  if (!args || args.length === 0) {
    ...
    for (const server of servers) {
      ...
      output.push(
        `  ${server.ip.padEnd(15)} - ${server.name.padEnd(15)} ${status}`
      );
      if (attempt && !cracked) {
  ...
  if (firstArg === "status") {
    ...
    if (target) {
      ...
      for (const attempt of attempts) {
        output.push(
          `  ${attempt.target.padEnd(15)} - ${attempt.attempts}/${
            attempt.maxAttempts
          } attempts`
        );
        if (attempt.mask) {
  ...
  if (firstArg === "clear") {
    ...
    return [
      "",
      target ? `Cleared attempts for ${target}.` : "Cleared all attempts.",
      "",
    ];
  }
  ...
  if (result.attempt) {
    output.push(
      `  Attempts: ${result.attempt.attempts}/${result.attempt.maxAttempts}`
    );
