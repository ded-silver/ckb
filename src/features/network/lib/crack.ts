import { unlockFile } from "@features/missions/model";
  ...
  for (let i = 0; i < guessUpper.length && i < passwordUpper.length; i++) {
    if (mask[i] === "_") {
      for (let j = 0; j < passwordUpper.length; j++) {
        if (!passwordMatched[j] && passwordUpper[j] === guessUpper[i] && j !== i) {
          mask[i] = "?";
...
export const attemptCrack = (
  ...
  const maxAttempts = server.difficulty === "easy" ? 5 : server.difficulty === "medium" ? 8 : 12;
  ...
  if (passwordUpper === serverPasswordUpper) {
    ...
    import("@features/email/lib")
      .then(module => {
        module.triggerVolkovWarning();
        module.triggerAchievementFirstHack();
      })
      .catch(error => {
  ...
  return {
    success: false,
    mask,
    message: `Access denied. ${remaining} attempt${remaining !== 1 ? "s" : ""} remaining.`,
    attempt,
  };
};
