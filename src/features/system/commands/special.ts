...
import type { Theme, CommandResult, TerminalSize, UserInfo } from "../../../types";
...
export const handleHistoryCommand = (
  ...
  if (args && args.length > 0) {
    ...
    const filtered = rawHistory.filter(cmd => cmd.toLowerCase().includes(searchTerm));
...
export const handleThemeCommand = (
  ...
  return {
    output: [`Invalid theme: ${newTheme}`, `Available: ${THEMES.join(", ")}`, ""],
    isError: true,
  };
};
...
export const handleSizeCommand = (
  ...
  if (!args || args.length === 0) {
    return {
      output: [
        `Current terminal size: ${currentSize?.width || 800}x${currentSize?.height || 600}`,
        "",
...
export const handleSuCommand = (
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(newUsername)) {
    return {
      output: ["Error: Username can only contain letters, numbers, underscores, and hyphens", ""],
      isError: true,
    };
  }
...
export const handleConfigCommand = (
  ...
  return {
    output: [
      ...
      `User: ${currentUserInfo?.username || "user"}@${currentUserInfo?.hostname || "cyberpunk"}`,
...
export const handleAntivirusCommand = (args?: readonly string[]): CommandResult => {
