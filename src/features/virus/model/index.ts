export type VirusType = "trojan" | "prototype" | "honeypot" | "adware" | "corruption";
...
export const setVirusState = (infected: boolean, virusType: VirusType = "trojan"): void => {
  if (infected) {
    ...
    if (virusType === "corruption") {
      import("@shared/lib/commandTracking").then(module => {
        if (
          module &&
          typeof module === "object" &&
          "ensureCorruptionDeactivationFile" in module &&
          typeof module.ensureCorruptionDeactivationFile === "function"
        ) {
          module.ensureCorruptionDeactivationFile();
        }
      });
    }
  } else {
...
export const checkVirusTrigger = (command: string, args?: string[]): boolean => {
  ...
  if (executionCommands.some(execCmd => cmd === execCmd || cmd.startsWith(execCmd))) {
    if (args && args.length > 0) {
      const fileArg = args.join(" ").toLowerCase();
      if (fileArg.includes("virus_prototype") || fileArg.includes("virus_prototype.asm")) {
        return true;
      }
    }
  }
...
export const detectVirusType = (_command: string, args?: string[]): VirusType => {
  const fileArg = args?.join(" ").toLowerCase() || "";
  ...
  if (fileArg.includes("message_from_lain") || fileArg.includes("message_from_")) {
    return "adware";
  }
...
export const checkDeactivationCode = (code: string, virusType?: VirusType): boolean => {
  if (!virusType) {
...
export const getVirusInfectionOutput = (virusType: VirusType = "trojan"): string[] => {
