...
import { getFilteredCommandHandler, CommandContext } from "./commandRegistry";
...
> = {
  ...
  list: (args) => {
    ...
    if (args.length >= 2 && args[0].toLowerCase() === "search") {
      ...
      if (results.length === 0) {
        ...
        results.forEach((track) => {
          ...
          const marker =
            playlistIndex === musicPlayer.getState().currentIndex ? ">" : " ";
          output.push(
            `${marker} ${playlistIndex + 1}. ${track.title} - ${track.artist}`
          );
        });
        output.push("");
        return output;
    ...
    const output = [
      `Playlist (${playlist.length} tracks, page ${page}/${totalPages}):`,
      "",
    ];
    ...
    if (totalPages > 1) {
      output.push("");
      output.push(
        `Use "list ${page < totalPages ? page + 1 : page}" for next page`
      );
    }
    output.push("");
    return output;
  ...
  stats: () => {
    ...
    if (stats.currentDuration > 0) {
      output.push(
        `Current track: ${formatTime(stats.currentPosition)} / ${formatTime(
    ...
    if (stats.artists.length > 0) {
      ...
      stats.artists.forEach((artist) => {
        output.push(
          `  ${artist.name}: ${
            artist.count
          } track(s) (${artist.percentage.toFixed(1)}%)`
        );
      });
    }
