...
const MIN_COMMANDS_AFTER_CONTACTS = 5;
...
export const incrementCommandsAfterContacts = (): number => {
  try {
    const current = parseInt(
      localStorage.getItem(COMMANDS_AFTER_CONTACTS_KEY) || "0",
      10
    );
...
export const shouldSendLainMessage = (): boolean => {
  ...
  try {
    const commandsCount = parseInt(
      localStorage.getItem(COMMANDS_AFTER_CONTACTS_KEY) || "0",
      10
    );
    const readTime = parseInt(
      localStorage.getItem(CONTACTS_READ_TIME_KEY) || "0",
      10
    );
    const elapsed = Date.now() - readTime;

    const shouldSend =
      commandsCount >= MIN_COMMANDS_AFTER_CONTACTS || elapsed >= MAX_WAIT_TIME;

    if (shouldSend) {
      const probability = Math.min(0.3 + commandsCount * 0.1, 0.9);
...
export const createLainMessageFile = (): void => {
  try {
    import("./filesystem").then(({ getFileSystem, saveFileSystem }) => {
      createLainMessageFileInternal(getFileSystem, saveFileSystem);
    });
  } catch (e) {
...
const createLainMessageFileInternal = (
  getFileSystem: () => Record<string, any>,
  saveFileSystem: (fs: Record<string, any>) => void
): void => {
  try {
