...
class MusicPlayer {
    ...
    if (this.audioContext && this.audioElement) {
      try {
        ...
        if (!this.analyser) {
          ...
          this.analyser.fftSize = 256; // Размер FFT для анализа
  ...
  // Генерация перемешанных индексов
  ...
  // Получить следующий индекс с учетом shuffle
  ...
  // Получить предыдущий индекс с учетом shuffle
  ...
  // Обработка окончания трека с учетом repeat
  ...
  // Переход к треку по номеру или имени
  async jumpToTrack(indexOrName: string | number): Promise<{
    ...
    if (typeof indexOrName === "number" || !isNaN(Number(indexOrName))) {
      const index =
        typeof indexOrName === "number" ? indexOrName : Number(indexOrName);
  ...
  // Получить следующий трек (без переключения)
  ...
  // Получить audio элемент
  getAudioElement(): HTMLAudioElement | null {
    return this.audioElement;
  }

  // Получить данные частот для визуализации
  ...
  // Получить количество частотных диапазонов
  ...
  private saveSettings(): void {
    try {
      ...
      const existingSettings: Partial<MusicPlayerSettings> = saved
        ? JSON.parse(saved)
        : {};
      ...
      const settings: MusicPlayerSettings = {
        ...
        lastTrack:
          this.currentIndex >= 0
            ? this.playlist[this.currentIndex]?.filename
            : undefined,
        lastPosition:
          this.status === "paused" && this.audioElement
  ...
  saveWindowState(
    ...
    try {
      const saved = localStorage.getItem(this.STORAGE_KEY);
      const settings: Partial<MusicPlayerSettings> = saved
        ? JSON.parse(saved)
        : {};
  ...
  // Получение статистики плейлиста
  getStats(): {
    totalTracks: number;
  ...
  // Добавление трека в историю воспроизведения
  ...
  // Получение истории воспроизведения
  getHistory(limit?: number): MusicTrack[] {
    const historyFilenames = limit
      ? this.playbackHistory.slice(0, limit)
